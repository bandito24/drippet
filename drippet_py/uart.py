...
class Uart:
    ...
    def extract_frame(self, start_index: int, buffer: bytes) -> NextFrameIndex:
        while buffer[start_index] != constants.START_BYTE and start_index < len(buffer) - 1:
            start_index += 1
        ...
        command: constants.Command = constants.Command(buffer[start_index + constants.HeaderOrder.COMMAND])
        data = []
        if(data_length != 0):
            ...
            for i in range(data_start_index, data_end_index - 1, 2):
                ...
                data.append(uint16)
        crc_check = self.crc_calc.checksum(buffer[(start_index + 1) : (frame_end - 2)]) #Skips the start bit and the 2 crc bits at the end 
        crc_received = int.from_bytes(buffer[frame_end - 2 : frame_end], byteorder="big")
